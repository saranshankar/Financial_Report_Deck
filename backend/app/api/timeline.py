...
def get_timeline(
    ...
    logger.info(f"Generating activity timeline for user: {current_user.id}")
    try:
        ...
        for tx in transactions:
            ...
            events.append(TimelineEvent(
                ...
                amount=tx.amount,
                date=normalize_datetime(tx.date),
            ...
            if limit > 0:
                ...
                if pct >= 100:
                    events.append(TimelineEvent(
                        ...
                        amount=spent - limit,
                        date=normalize_datetime(datetime.utcnow() - timedelta(minutes=5)),
                        status="WARNING"
                    ))
                elif pct >= 80:
                    events.append(TimelineEvent(
                        ...
                        amount=limit - spent,
                        date=normalize_datetime(datetime.utcnow() - timedelta(minutes=10)),
                        status="WARNING"
                    ))
        ...
        for c in contributions:
            ...
            events.append(TimelineEvent(
                ...
                date=normalize_datetime(c.created_at),
                status="SUCCESS"
            ))
        ...
        for sub in subscriptions:
            ...
            events.append(TimelineEvent(
                ...
                date=normalize_datetime(renewal_date),
                status="INFO"
            ))
        ...
        logger.error(f"Error compiling timeline feed: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to compile activity timeline: {str(e)}"
        )
